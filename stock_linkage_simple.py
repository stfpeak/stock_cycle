...
function switchTab(tab) {
    ...
    else if (tab === 'deepsearch') document.querySelectorAll('.tab')[6].classList.add('active');
    else if (tab === 'stockquery') document.querySelectorAll('.tab')[7].classList.add('active');
    else if (tab === 'recommend') document.querySelectorAll('.tab')[8].classList.add('active');
    else if (tab === 'stats') document.querySelectorAll('.tab')[9].classList.add('active');
    ...
    if (el) {
        ...
        if (code) {
            if (currentTab === 'npattern') {
                // N字战法卡片也打开K线弹框，支持导航浏览
                var tabContainer = document.querySelector('#np-nav-top, .result') || document;
                var stockEls = tabContainer.querySelectorAll('[data-code]');
                var stocks = [];
                var seenCodes = {};
                stockEls.forEach(function(s) {
                    var sc = s.getAttribute('data-code');
                    var sn = s.getAttribute('data-name') || sc;
                    if (sc && !seenCodes[sc]) {
                        seenCodes[sc] = true;
                        stocks.push({code: sc, name: sn});
                    }
                });
                var clickedIdx = -1;
                for (var si = 0; si < stocks.length; si++) {
                    if (stocks[si].code === code) { clickedIdx = si; break; }
                }
...
function loadRecommend() {
    ...
    _cachedFetch('/api/recommend?top_n=30').then(function(data) {
...
function loadNPattern() {
    ...
    _cachedFetch('/api/n_pattern').then(function(data) {
...
function loadLinkageDefaultSections() {
    ...
    Promise.all([
        ...
        var todayZt = results[1] || [];
        var hotStocks = results[2] || [];
        var hotRank100 = results[3] || [];
        var hotConcepts = results[4] || [];
        ...
        // 连板天梯 Top 10
        html += '<div class="rt-section">';
        html += '<h3>🏆 连板天梯 Top 10</h3>';
        html += renderLianbanLadderTable(ladder);
        html += '</div>';

        // 今日涨停 Top 20
        html += '<div class="rt-section">';
        html += '<h3>⚡ 今日涨停 Top 20</h3>';
        html += renderTodayZtList(todayZt.slice(0, 20));
        html += '</div>';
        ...
        // === 同花顺热股Top100 ===
        html += '<div class="rt-section">';
        html += '<h3>🔥 同花顺热股Top100 <span class="count-badge">' + hotRank100.length + '只</span></h3>';
        html += renderHotRank100Table(hotRank100);
        html += '</div>';

        // === 同花顺热门概念板块Top20 ===
        html += '<div class="rt-section">';
        html += '<h3>📈 同花顺热门概念板块Top20 <span class="count-badge">' + hotConcepts.length + '个</span></h3>';
...
function _kplRenderL1(l1Key, l1Data) {
    ...
    for (var i = 0; i < l2Keys.length; i++) { html += _kplRenderL2(l2Keys[i], l1Data[l2Keys[i]]); }
    html += '</div></div>';
    return html;
}

function _kplRenderL2(l2Key, items) {
    var stockCount = 0;
    ...
    html += '<span class="kpl-expand-btn" onclick="event.stopPropagation();_kplToggleSubtree(this)" title="展开/收起全部">&#xb1;</span>';
    html += '</div><div class="kpl-children">';
    for (var i = 0; i < items.length; i++) { html += _kplRenderL3(items[i]); }
    html += '</div></div>';
    return html;
}

function _kplRenderL3(item) {
...
function renderStockQueryPage(data, code, name) {
    var h = '<div class="sq-header"><h2>' + (name || '') + '</h2><span class="sq-code">' + code + '</span></div>';
    ...
    h += '</div>';
    // Query linkage button
    h += '<div style="text-align:center;padding:16px 0;">';
    h += '<button onclick="switchTab(\\x27linkage\\x27);setTimeout(function(){document.getElementById(\\x27linkageStockInput\\x27).value=\\x27' + code + '\\x27;doLinkageSearch();},100)" style="background:#00d4ff;color:#0a1628;border:none;padding:8px 24px;border-radius:6px;font-size:0.95em;font-weight:600;cursor:pointer;">查询联动</button>';
    h += '</div>';
    return h;
}
    ...
    def do_GET(self):
        ...
        if path == '/' or path == '/index.html':
            ...
            try:
                top_n = int(query.get('top_n', ['30'])[0])
                refresh = query.get('refresh', [None])[0]
                cache_key = f'recommend_15_{top_n}'
                recs = None if refresh else _get_cached(cache_key)
                if recs is None:
                    recs = finder.recommend_pullback_stocks(lookback_days=15, top_n=top_n)
                    _set_cache(cache_key, recs)
                self._respond_json({
